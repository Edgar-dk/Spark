...
/*一：页面转化率
     精确提取数据
*    */
object Spark_Req_PageFlowPercentConversionPrecise {
  def main(args: Array[String]): Unit = {
    val SparkConf = new SparkConf().setMaster("local[*]").setAppName("Spark_Req_PageFlowPercentConversionPrecise")
    ...
    *   提取分母，直接把页面点击数量获取出来，init，不包含，最后一个数，因为
    *   最后一个数，不做分母*/
    val pageIdToCount: RDD[(Long, Long)] = actionDataRdd.filter(
      action => {
